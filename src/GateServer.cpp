...
//redis默认监听端口为6387 可以再配置文件中修改
    ...
    redisReply* r = (redisReply*)redisCommand(c, "AUTH %s", redis_password.c_str());//redis认证转换为c风格字符串
...
int main()
{   
    //Test_RedisPool();//测试redis
    auto&gCfgMgr=ConfigMgr::Inst();//单例模式，用引用是因为，单例模式没有拷贝构造和赋值运算符重载
